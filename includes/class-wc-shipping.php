...
class WC_Shipping {
	...
	public function calculate_shipping_for_package( $package = array(), $package_key = 0 ) {
		...
		$stored_package_hash = isset( $stored_rates['package_hash'] ) ? $stored_rates['package_hash'] : '';

		if ( ! is_array( $stored_rates ) || $package_hash !== $stored_package_hash || 'yes' === get_option( 'woocommerce_shipping_debug_mode', 'no' ) ) {
